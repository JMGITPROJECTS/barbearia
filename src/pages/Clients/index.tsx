...
import {getClientes,addCliente, deleteCliente, updateCliente,} from "../../services/clientService";
...
function Clientes() {
  ...
  const handleSubmit = (valores: { nome: string, cpf: string, telefone: string, email:string}) => {
    if(editandoCliente) {
        ...
        const resultado = addCliente(valores.nome, valores.cpf, valores.telefone, valores.email);
        if(!resultado.sucesso){
            message.error(resultado.mensagem);
        } else{
            message.success(resultado.mensagem);
    ...
    <div style={{ padding: 24 }}>
      ...
      >
        <Form form={form} onFinish={handleSubmit}>
          ...
          >
            <Input maxLength={15} 
            onChange={(e) => form.setFieldValue('telefone', formatarTelefone(e.target.value))}/>
          </Form.Item>
